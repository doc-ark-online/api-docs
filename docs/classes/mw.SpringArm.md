...
使用示例:创建一个名为"Example_Camera_SpringArm_CollisionEnabled"的脚本,放置在对象栏中,打开脚本,输入以下代码保存,运行游戏,你将在场景中生成10根柱子用作触发摄像机弹簧杆碰撞，按下键盘“1”，启用/禁用摄像机弹簧杆碰撞。你可以看到禁用摄像机弹簧杆碰撞摄像机碰撞柱子不同的效果.代码如下:
```ts
@Core.Class
export default class Example_Camera_SpringArm_CollisionEnabled extends Script {
// 预加载使用到的资源
@Core.Property()
preloadAssets = "26950";
...
使用示例:创建一个名为"Example_Camera_SpringArm_Length"的脚本,放置在对象栏中,打开脚本,输入以下代码保存,运行游戏,按住键盘“3”，增加摄像机弹簧臂的长度，按住键盘“4”，减少摄像机弹簧臂的长度.你将在场景中看到摄像机弹簧杆伸缩的效果.代码如下:
```ts
@Core.Class
export default class Example_Camera_SpringArm_Length extends Script {
    // 当脚本被实例后，会在第一帧更新前调用此函数
    protected onStart(): void {
        // 下列代码仅在客户端执行
        if(SystemUtil.isClient()) {
            ...
            InputUtil.onKeyPress(Type.Keys.Four, () => {
                myCamera.springArm.length -= 1;
            });
        }
    }
...
使用示例:创建一个名为"Example_Camera_SpringArm_LocalTransform"的脚本,放置在对象栏中,打开脚本,输入以下代码保存,运行游戏,按下键盘“1”，切换摄像机弹簧臂的偏移(0, 100, 100)，2秒后复原.你将在场景中看到摄像机偏移的效果并在控制台看到打印的变化后的摄像机弹簧臂的本地变换.代码如下:
```ts
@Core.Class
export default class Example_Camera_SpringArm_LocalTransform extends Script {
    // 当脚本被实例后，会在第一帧更新前调用此函数
    protected onStart(): void {
        // 下列代码仅在客户端执行
        if(SystemUtil.isClient()) {
...
使用示例:创建一个名为"Example_Camera_SpringArm_MovementCollisionDetectionEnabled"的脚本,放置在对象栏中,打开脚本,输入以下代码保存,运行游戏,你将在场景中生成10根柱子用作触发摄像机弹簧杆碰撞，开启摄像机弹簧杆碰撞后，按下键盘“2”，启用/禁用摄像机弹簧杆移动碰撞检测。你可以看到禁用摄像机弹簧杆移动碰撞检测后摄像机碰撞柱子不同的效果.代码如下:
```ts
@Core.Class
export default class Example_Camera_SpringArm_MovementCollisionDetectionEnabled extends Script {
// 预加载使用到的资源
@Core.Property()
preloadAssets = "26950";
    // 当脚本被实例后，会在第一帧更新前调用此函数
...
使用示例:创建一个名为"Example_Camera_SpringArm_UseControllerRotation"的脚本,放置在对象栏中,打开脚本,输入以下代码保存,运行游戏,按下键盘“2”，启用/禁用控制器操作摄像机的旋转5秒.你将在场景中看到禁用控制器操作摄像机后的无法控制摄像机旋转的效果.代码如下:
```ts
@Core.Class
export default class Example_Camera_SpringArm_UseControllerRotation extends Script {
    // 当脚本被实例后，会在第一帧更新前调用此函数
    protected onStart(): void {
        // 下列代码仅在客户端执行
        if(SystemUtil.isClient()) {
...
使用示例:创建一个名为"Example_Camera_SpringArm_WorldTransform"的脚本,放置在对象栏中,打开脚本,输入以下代码保存,运行游戏,按下键盘“1”，切换摄像机弹簧臂的偏移(0, 100, 100)，2秒后复原.你将在场景中看到摄像机偏移的效果并在控制台看到打印的变化后的摄像机弹簧臂的世界变换.代码如下:
```ts
@Core.Class
export default class Example_Camera_SpringArm_WorldTransform extends Script {
    // 当脚本被实例后，会在第一帧更新前调用此函数
    protected onStart(): void {
        // 下列代码仅在客户端执行
        if(SystemUtil.isClient()) {
