[Physics](../groups/Physics.Physics.md) / PhysicsFulcrum
...
使用示例:(不支持动态生成)
```ts
let test = Core.GameObject.find(GUID) as Gameplay.PhysicsFulcrum;
test.attachToGameObject(gameObject);
test.limitAngle = new Type.Vector(100,100,100);
test.enable = true;
```
