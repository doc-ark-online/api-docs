...
tri.onEnter.add((obj: Core.GameObject) => {
});
tri.onLeave.add((obj: Core.GameObject) => {
});
```
...
| **[isInArea](Gameplay.Trigger.md#isinarea)**(`gameObject`: `GameObject`): `boolean` <br> 判断指定对象是否在触发器区域|
| **[isSphereShape](Gameplay.Trigger.md#issphereshape)**(): `boolean` <br> 判断当前触发器区域是否为球体|
| **[setBoxExtent](Gameplay.Trigger.md#setboxextent)**(`InBoxExtent`: [`Vector`](Type.Vector.md), `updateOverlaps?`: `boolean`): `void` <br> 设置立方体触发器大小|
| **[setCollisionEnabled](Gameplay.Trigger.md#setcollisionenabled)**(`enable`: `boolean`): `void` <br> 设置触发器是否激活|
| **[setSphereRadius](Gameplay.Trigger.md#setsphereradius)**(`InSphereRadius`: `number`, `updateOverlaps?`: `boolean`): `void` <br> 设置球形触发器大小|
