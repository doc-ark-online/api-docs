...
| **[getPlayerID](Gameplay.Player.md#getplayerid)**(): `number` <br> 获取当前游戏内的玩家Id，用于和其它玩家区分。不能用于查询用户信息，因为每次重进游戏时此ID会改变，如果需要获取玩家的唯一Id请使用[getUserSystemId]|
...
| **[getUserSystemId](Gameplay.Player.md#getusersystemid)**(): `string` <br> 获取用户系统的玩家Id,玩家唯一Id|
