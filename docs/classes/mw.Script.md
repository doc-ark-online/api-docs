...
| `path` `string` | 属性路径<br> range: 依据路径长度而定 |
| :------ | :------ |
| `value` `unknown` | 属性值 |
...
| `dt` `number` | 与上一帧的延迟 单位:秒  <br> range: dt 的大小会根据游戏性能发生变化，游戏性能下降，帧率可能会下降，从而导致 onUpdate 函数的调用频率减少。（详情请看类开头描述）<br> type:浮点类型数值 |
| :------ | :------ |


setUpdate 设置为 true 后，每帧被执行，设置为 false，不会执行
