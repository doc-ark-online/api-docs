[Gameplay](../groups/Gameplay.Gameplay.md) / VehicleCameraSetting

# VehicleCameraSetting <Badge type="tip" text="Class" /> <Score text="VehicleCameraSetting" />

载具摄像机，对应编辑器对象管理器中载具下挂载的摄像机对象
             它是由摄像机系统接口封装的游戏对象，为载具提供所需的镜头控制效果
             注意:目前游戏中只存在一个摄像机，所以本质上使用的还是角色下的摄像机
...
| Methods |
| :-----|
| **[applyClientSettings](Gameplay.VehicleCameraSetting.md#applyclientsettings)**(`cameraSystem`: [`CameraSystem`](Gameplay.CameraSystem.md)): `void` <br> 应用面板的摄像机配置数据|
...
• **applyClientSettings**(`cameraSystem`): `void` <Badge type="tip" text="client" />
...
• **getSettings**(): [`CameraSystemData`](../modules/Gameplay.Gameplay.md#camerasystemdata) <Badge type="tip" text="client" />
...
• **saveSettings**(): `void` <Badge type="tip" text="client" />
