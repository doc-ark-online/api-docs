...
| `bPause?` `boolean` | 设置暂停状态  <br> default: true |
| :------ | :------ |
...
| `startTime?` `number` | 设置起始播放时间  <br> default: 0<br> range: 不大于音效的时长<br> type: 浮点数 |
| :------ | :------ |
| `onSuccess?` () => `void` | 播放完成后回调  <br> default:  null |
...
| `assetGUID` `string` | 音效资源 ID<br> range: 依据 ID 长度而定 |
| :------ | :------ |
