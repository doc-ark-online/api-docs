...
```ts
@Core.Class
export default class TweenExample extends Core.Script {
...
class DaoDanScript {
    ...
    //导弹预制体
    private prefab: Gameplay.GameObject
    ...
    private async init(bornPos: Vector, toPos: Vector) {
        ...
        Core.GameObject.asyncSpawn({ guid: bombAssetId }).then(obj => {
            obj.attachToGameObject(this.prefab);
            obj.relativeLocation = new Vector(0, 0, 0);
            obj.relativeRotation = new Rotation(0, -90, 0);
        })
        EffectService.getInstance().playEffectOnGameObject(fireAssetId, this.prefab, 0, new Type.Vector(0, 0, 0), new Type.Rotation(0, -90, 0), new Type.Vector(2, 2, 2));
...
CatmullRom插值：拐弯平滑，只在拐弯处进行平滑
    ...
    private fireReadyAnim() {
        ...
        let tweenB = new TweenUtil.Tween(startPos).to(startPos.clone().add(new Type.Vector(0, 0, Math.random() * 100)), 1000).onUpdate((value) => {
