 ...
 * @returns 
 */
function getCustomConfigForTheme(): any {
  const vsrsions = Utils.parseVersionConfig();
  if (vsrsions.length <= 0) {
    console.error("version.json is empty, no version is configed.")
    return {};
  }

  /** 当前版本号 */
  const currentVersion = vsrsions[0].text;

  /** 是否为测试版本 */
  const isBetaVersion = vsrsions[0].isBetaVersion ?? false;

  /** 链接的版本 */
  const LiknedVersions = vsrsions.slice(1);
  LiknedVersions.forEach((entry) => {
    entry.text = Utils.getDisplayVersionText(entry.text, entry.isBetaVersion ?? false);
  });
...
export default defineConfigWithTheme<DefaultTheme.Config>(getCustomConfigForTheme())
